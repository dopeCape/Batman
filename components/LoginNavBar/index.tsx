...
export default function LoginNavBar() {
    ...
    return (
      <div className='flex h-full items-center'>
        ...
        <Link className={`flex flex-row items-center ${isActive ? 'h-full items-center flex text-center px-3 dark:text-white border-b-4 dark:border-white border-[#3247CF] font-bold text-[#3247CF]' : 'dark:text-white text-[#3E4045] font-normal'
          ...
          {stroke &&
            <svg xmlns="http://www.w3.org/2000/svg" width={isActive ? "20" : "15"} height={isActive ? "20" : "15"} viewBox="0 0 25 25" fill="none">
              <path d="M9.43831 20.5968H6.40207C5.32839 20.5968 4.29869 20.1703 3.53948 19.4111C2.78028 18.6519 2.35376 17.6222 2.35376 16.5485V7.43979C2.35376 6.36611 2.78028 5.3364 3.53948 4.5772C4.29869 3.81799 5.32839 3.39147 6.40207 3.39147H17.5349C18.6086 3.39147 19.6383 3.81799 20.3975 4.5772C21.1567 5.3364 21.5833 6.36611 21.5833 7.43979V10.476M8.42623 2.37939V4.40355M15.5108 2.37939V4.40355M2.35376 8.45187H21.5833M19.0531 16.1872L17.5349 17.7053" stroke={(theme === 'dark' && isActive) ? 'white' : (theme === 'light' && isActive) ? "#3247CF" : "#A8AAB0"} stroke-width="2.02416" stroke-linecap="round" stroke-linejoin="round" />
              <path d="M17.5313 22.6203C20.3261 22.6203 22.5917 20.3547 22.5917 17.5599C22.5917 14.7651 20.3261 12.4995 17.5313 12.4995C14.7366 12.4995 12.4709 14.7651 12.4709 17.5599C12.4709 20.3547 14.7366 22.6203 17.5313 22.6203Z" stroke={(theme === 'dark' && isActive) ? 'white' : (theme === 'light' && isActive) ? "#3247CF" : "#A8AAB0"} stroke-width="2.02416" stroke-linecap="round" stroke-linejoin="round" />
            </svg>
          }
          {!stroke && <svg xmlns="http://www.w3.org/2000/svg" className={isActive && className} width={isActive ? "20" : "15"} height={isActive ? "20" : "15"} viewBox="0 0 25 25" fill={(theme === 'dark' && isActive) ? 'white' : (theme === 'light' && isActive) ? "#3247CF" : "#A8AAB0"}>
            <path d={d} fill="" />
          </svg>}
          {children}
        </Link>
  ...
  return (
    <div className='w-screen h-20 dark:bg-[#1B1D21] flex bg-[#fff] flex-row items-center justify-around border-b-[#A8AAB0] border-b-2 border-opacity-20 '>
      ...
      <div className='flex flex-row items-center justify-around w-full h-full'>
        ...
        <Link href="/pricing" className={`bg-gradient-to-r  from-[#00C5D7] to-[#0077BE] rounded-md p-2 flex flex-row justify-between text-white`}>
        ...
        <div className='relative bg-slate-300  w-10 rounded-full items-center justify-center flex'>
          <div onClick={toggleLogout} className='rounded-full p-1 flex items-center justify-center cursor-pointer'>

            <h1 className='text-2xl w-full h-full'>{User?.email?.charAt(0).toUpperCase()}</h1>
          </div>
          {showSettings && <div className={`flex flex-col absolute  p-2 mt-44 w-28  h-32 bg-gray-600`}>
