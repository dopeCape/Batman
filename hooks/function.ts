export function setPrompt(
  ...
  if (
    ...
    return `Generate one scroll-stopping ${title} for the topic ${input}, ${
    ...
    return `Generate one an impressive YouTube video description that showcases ${input} ${
    ...
    return `Generate one scroll stopping ${title} for a video about ${input}. ${
      keywords ? `for the keywords ${keywords}` : null
    }.${tone ? `with the tone ${tone}` : null}, ${
    ...
    return `Generate one an impressive ${title}  that showcases ${input} ${
      tone ? `in a tone [Tone]` : null
    }, ${keywords ? `with keywords ${keywords}` : null}  ${
      targetAudience ? `targetting a target audience ${targetAudience}` : null
    } and attracts followers.`
  } else if (title == "LinkedIn Post") {
    return `Generate an engaging LinkedIn  Post ${
      postType ? `of the type ${postType}` : null
    }, ${
      industry ? `related to the industry ${industry}` : null
    } that showcases [Whats the post is about] ${
      tone ? `with the tone [Tone]` : null
    },  ${keywords ? `with keywords ${keywords}` : null} ${
      targetAudience ? `targetting a target audience ${targetAudience}` : null
    }. `
    ...
    return `Generate an attention-grabbing Tweet on the topic ${input}, ${
      ...
      keywords ? `with keywords ${keywords}` : null
    } that resonate with your followers and gain retweets. `
  } else if (title == "Facebook Page Description") {
    return `Generate an impressive Facebook Page Description description that showcases ${input} ${
      tone ? `in a tone [Tone]` : null
    },  ${keywords ? `with ${keywords}` : null} ${
      targetAudience ? `targeting the audience ${targetAudience}` : null
    } and attracts followers.`
  } else if (title == "Facebook Post") {
    return `Generate an engaging Facebook Post that showcases ${input} ${
      tone ? `in a tone [Tone]` : null
    },${keywords ? `with ${keywords}` : null} ${
      targetAudience ? `targeting the audience ${targetAudience}` : null
    } that sparks conversations and drives interactions.`
  } else if (title == "Facebook Ad Copy") {
    return `Generate a compelling Facebook Ad copy about ${input}, ${
      tone ? `in a tone [Tone]` : null
    }, ${keywords ? `with ${keywords}` : null}, ${
    ...
    return `Generate an engaging LinkedIn  Article ${
      postType ? `of the type ${postType}` : null
    }, ${
      industry ? `related to the industry ${industry}` : null
    } that showcases [Whats the post is about] ${
...
export function Descriptions(title: string) {
  ...
  } else if (title == "YouTube Video Ideas" || title == "YouTube Video") {
